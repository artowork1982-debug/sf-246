...
try {
    ...
    if ($id > 0) {
        ...
        try {
            ...
            // Fetch current state/type for resubmission logic (don't trust POST for state)
            $stmt = $pdo->prepare("SELECT state, type FROM sf_flashes WHERE id = ? FOR UPDATE");
            $stmt->execute([$id]);
    ...
    // =========================================================================
        ...
        $sql = "INSERT INTO sf_flashes
            (title, title_short, summary, description, type, site, site_detail, occurred_at, lang, state, created_by,
             ...
             selected_approvers, preview_status, font_size_override, created_at, updated_at)
            VALUES
            (:title, :title_short, :summary, :description, :type, :site, :site_detail, :occurred_at, :lang, :state, :created_by,
             ...
             :image1_caption, :image2_caption, :image3_caption,
             :selected_approvers, 'pending', :font_size_override, NOW(), NOW())";
        ...
        $stmt->execute([
            ...
            ':grid_bitmap'      => '', // Temporary empty value, will be updated below
