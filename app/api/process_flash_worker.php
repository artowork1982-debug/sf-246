...
try {
    ...
    try {
        ...
        // Determine if green type needs two slides
        $needsTwoSlides = false;
        if ($type === 'green') {
            require_once __DIR__ . '/../services/PreviewImageGenerator.php';
            
            $flashDataForCheck = [
                'title_short'        => $rendererData['short_text'],
                'description'        => $rendererData['description'],
                'root_causes'        => $rendererData['root_causes'],
                'actions'            => $rendererData['actions'],
                'font_size_override' => $rendererData['font_size_override'] ?? null,
            ];
            
            $tempPreviewsDir = __DIR__ . '/../../uploads/previews/';
            $tempUploadsDir  = __DIR__ . '/../../uploads/';
            $imgGen = new PreviewImageGenerator(null, $tempUploadsDir, $tempPreviewsDir);
            $needsTwoSlides = $imgGen->needsSecondCardPublic($flashDataForCheck);
        }
