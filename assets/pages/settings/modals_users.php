...
(function () {
    ...
    // LOMAKKEEN SUBMIT (delegoitu) - Handle single unified form
    document.addEventListener('submit', function (e) {
        ...
        if (e.target.closest('#sfUserForm')) {
            ...
            var apiUrl = userId 
                ? '<?= $baseUrl ?>/app/api/users_update.php' 
                : '<?= $baseUrl ?>/app/api/users_create.php';
            ...
            fetch(apiUrl, {
                ...
                if (data.ok) {
                    ...
                    alert(data.error || '<?= htmlspecialchars(sf_term('error_save_failed', $currentUiLang) ?? 'Tallennus epäonnistui', ENT_QUOTES, 'UTF-8') ?>');
                }
            }).catch(function (err) {
                console.error('Network error:', err);
                alert('<?= htmlspecialchars(sf_term('error_network', $currentUiLang) ?? 'Verkkovirhe', ENT_QUOTES, 'UTF-8') ?>: ' + err.message);
            });
            return;
        }
    });
