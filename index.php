...
(function() {
    ...
    // Automatic keepalive request every 5 minutes if user is active
    let keepaliveInterval = setInterval(async function() {
        const now = Date.now();
        const elapsed = Math.floor((now - lastActivityTime) / 1000);
        
        // Only send keepalive if user was active within last 10 minutes
        // (small margin to ensure session doesn't expire)
        if (elapsed < 600) {
            try {
                const csrfInput = document.querySelector('input[name="csrf_token"]');
                const csrfToken = csrfInput ? csrfInput.value : '';
                ...
                if (response.ok) {
                    resetActivity();
                    console.log('[SessionKeepAlive] Session extended successfully');
                } else {
                    console.warn('[SessionKeepAlive] Failed to extend session, status:', response.status);
                }
            } catch (error) {
